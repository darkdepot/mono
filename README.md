...
`review-bench.mjs manifest|run|score|report` compares reviewers on frozen archive inputs without writing receipts or certificates. It installs with the [default candidate routes](scripts/review-bench-routes.mjs); provider key values come only from the environment. `manifest --dry-run --evidence-root DIR --repo DIR` inventories reproducible cases and explains exclusions. `run --dry-run --routes FILE --evidence-root DIR --repo DIR` checks admission without provider calls. A live route requires both a documented payment channel and the helper's offline compatibility checks; missing credentials and unverified routes are excluded.

The [sampling and blind-grading protocol](docs/review-bench-protocol.md) covers frozen code/base/dataset/helper inputs, task-family clusters, defect/fix pairs, held-out gold, repetitions, stop rules, operational failures and usage coverage. The common experiment disables web search and tools; the incumbent's production settings are a separate experiment. Results stay under `<evidenceRoot>/bench/<runId>/`. A thin archive yields feasibility evidence with no quality recommendation. Choosing a reviewer is a separate recorded decision; the bench never changes policy. Live provider runs and keys belong to the orchestrator after installation.
