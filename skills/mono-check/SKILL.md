...
- `post-ship`: a missing, failed, or skipped project update is never a `FAIL`. Project Updates are informational, `mono-deploy` owns them, and this mode neither requires one nor reads one as evidence.
