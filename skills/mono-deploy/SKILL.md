...
12. `project-update`: publish one project update per deployed Issue and, when this shipment was the project's last, complete the project. Form and text rules live in `templates/project-update.md`; this step owns the order, the reads, and the failure semantics.
    ...
    2. Project of the update. On the Project-first path it is the Issue's own project, and the sub-steps below run in full on it. For an issue-only Issue, or any Issue with no project, read the `Тематический проект:` line from the live Issue body — exactly one such line is valid, and two or more make the field malformed, which takes the ambiguous outcome below without any name lookup — and resolve the project it names by EXACT name among the projects of the Linear team in the project config, archived projects excluded. The outcomes are three:
       - Exactly one match: continue with sub-steps 3, 6, 7, and 8 on that project. Idempotency is by this Issue's chip; sub-steps 4 and 5 do not run at all; the form is the ordinary one and the completion prefix is never used; no project status is written — a project already in `Completed` receives the same ordinary update as an active one.
       - Zero or several matches, or a malformed field: end this step as `Project update: not posted — тематический проект «<имя>» не найден / неоднозначен`, quoting the field's own text when the field itself is what is ambiguous.
       - The line says `нет`, or the Issue carries no such line because it was approved before this rule existed: end this step as `Project update: n/a — тематический проект в Issue не назван`, or as `Project update: n/a — тематический проект: нет (<причина из Issue>)` when the Issue gives the reason.
       The `Project:` line on this path is `n/a — тематический проект «<имя>», статус не меняется` when a theme project was resolved, and `n/a — тематического проекта нет` when none was. An Issue closed without a deploy never reaches this step at all.
    3. Idempotency read: read the project's existing updates. When an update already carries this Issue's chip, publication is skipped as `already posted`, but the completion check and the status transition still run on the Project-first path; on the issue-only theme-project path sub-steps 4 and 5 do not run at all. Idempotency is by EFFECT, not by step: a skipped publication never cancels sub-steps 4 and 5 on the Project-first path; on the issue-only theme-project path sub-steps 4 and 5 do not run at all.
    ...
    6. Text: compose the update from `templates/project-update.md` (the «Выкладка» form; the «Состояние» form belongs to the orchestrator, never to this step) in the SAME pass as the closeout lead «Выкатили: …», so both name one result. The final form is permitted only under the sub-step 5 condition.
