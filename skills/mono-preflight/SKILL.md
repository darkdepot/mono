...
5. Run the mandatory `autoreview` gate:
   ...
   - Resolve the model from [role:autoreview](references/model-policy.md#roles) and the effort only from the canonical table in `references/autoreview-routing.md`; do not restate or infer a second copy of the table in this skill.
...
- Do not mark preflight `ready` when the final `autoreview` command omits explicit `--engine claude`, `--model`, or `--thinking`, runs an `--engine` other than `claude`, selects a model other than the identifier resolved from [role:autoreview](references/model-policy.md#roles), or does not match the final risk class in `references/autoreview-routing.md`.
