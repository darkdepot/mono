...
- `prerequisites.autoreviewHelper`: must be `true`; `mono-preflight` blocks if the helper is unavailable. The workflow passes an explicit model resolved from [role:autoreview](model-policy.md#roles) and effort from `references/autoreview-routing.md` instead of inheriting the external helper's default.
