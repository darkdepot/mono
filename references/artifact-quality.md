...
- Records the final risk source, model resolved from [role:autoreview](model-policy.md#roles), reasoning effort, and any upward reclassification. The final command must pass explicit `--engine claude`, `--model`, and `--thinking` values matching `references/autoreview-routing.md`.
